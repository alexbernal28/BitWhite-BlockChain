...
import { createTender, ApiError } from "../../lib/api";
import { getSession, type Session } from "../../lib/session";

const CATEGORIAS = ["Bienes", "Servicios", "Obras", "Consultoría"];
...
export default function NuevaLicitacionForm() {
  ...
  // Evita parpadeo mientras se lee la sesión desde localStorage.
      ...
      <div className="flex flex-1 items-center justify-center bg-gov-surface px-4 py-12">
        <div className="w-full max-w-md rounded-xl border border-gov-border bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm text-gov-ink-muted">
            Debes iniciar sesión con una cuenta institucional (gobierno) para publicar una licitación.
          </p>
          <Link
            href="/login"
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    formData.set("title", String(data.get("titulo") || ""));
    formData.set("processNumber", String(data.get("numeroProceso") || ""));
    formData.set("description", String(data.get("descripcion") || ""));
    formData.set("category", String(data.get("categoria") || ""));
    const budget = String(data.get("presupuesto") || "");
    if (budget) formData.set("budget", budget);
    formData.set("publicationDate", String(data.get("fechaPublicacion") || ""));
    formData.set("deadline", String(data.get("fechaLimite") || ""));
    const requirements = String(data.get("requisitos") || "");
    if (requirements) formData.set("requirements", requirements);
    const file = data.get("documentoPliego");
    if (file instanceof File && file.size > 0) formData.set("documentoPliego", file);

    setError(null);
    setLoading(true);
    try {
      await createTender(session.token, formData);
      router.push("/gobierno");
    } catch (err) {
      setError(err instanceof ApiError ? err.message : "No se pudo publicar la licitación.");
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="flex flex-1 justify-center bg-gov-surface px-4 py-12">
      <div className="w-full max-w-2xl rounded-xl border border-gov-border bg-white p-8 shadow-sm">
        ...
        <p className="mt-1 text-sm text-gov-ink-muted">
          Los datos ingresados crearán un registro en el modelo <code className="text-xs">Tender</code> del backend.
          Al publicar, se calcula el hash SHA-256 del pliego para su verificación posterior.
        </p>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <button
              type="submit"
              disabled={loading}
              className="flex-1 rounded-md bg-gov-blue-900 px-4 py-2.5 text-sm font-semibold text-white hover:bg-gov-blue-800 disabled:cursor-not-allowed disabled:opacity-60"
            >
              {loading ? "Publicando…" : "Publicar licitación"}
            </button>
            <Link
              href="/gobierno"
